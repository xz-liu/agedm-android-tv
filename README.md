...
- 设置页提供“重建 Bangumi 匹配索引”，会遍历本地已缓存的 AGE 动画元数据，重新搜索 Bangumi 构建 subject 语料，再在本地重跑 `AGE -> Bangumi` 匹配
...
- Bangumi 匹配索引重建：重新搜索本地已缓存的 AGE 动画，刷新 Bangumi subject 语料并批量重跑匹配
