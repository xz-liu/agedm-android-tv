...
- 动画详情页选择 **下载选集**，选中剧集后直接解析并下载，无需进入播放器；详情页 **下载管理** 定位到主界面的 **下载** 分区，在这里查看进度、暂停、继续、重试或删除
- 支持可由 Media3 下载的点播 HLS 和直链视频；HLS 使用选定轨道，避免把每个清晰度都下载一遍
- 一次下载一集，支持后台运行；应用被系统终止或电视重启后，打开下载页恢复未暂停的任务
- 下载完成后，从下载列表选择 **离线播放**，不依赖动画详情接口或在线解析；保存本地观看进度
- 下载保存在应用内部空间，与内容缓存分开；清理内容缓存不会删除视频，卸载应用会删除下载
- 开始前检查至少 256 MB 可用空间；该检查不保证整集容量足够，存储写满时任务会失败，可删除旧下载后重试
- 源地址过期时，删除失败任务，回到详情页重新选择该集下载；暂不支持批量下载、DRM 视频或直播
